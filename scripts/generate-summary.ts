...
function generateSummary(cocktails: Cocktail[]): Summary {
  ...
  for (const cocktail of cocktails) {
    ...
    for (const spirit of cocktail.base_spirits) {
      ...
      const info = baseSpiritCounts.get(key) || { count: 0, tags: new Set() };
      info.count++;
    ...
    for (const liqueur of cocktail.liqueurs) {
      ...
      const info = liqueurCounts.get(key) || { count: 0, tags: new Set() };
      info.count++;
    ...
    for (const ingredient of cocktail.ingredients) {
      ...
      const info = ingredientCounts.get(key) || { count: 0, tags: new Set() };
      info.count++;
    ...
    for (const flavor of cocktail.flavor_descriptors) {
      ...
      const info = flavorCounts.get(key) || { count: 0, tags: new Set() };
      info.count++;
  ...
  function createSortedItems(counts: Map<string, CountInfo>): SummaryItem[] {
    return Array.from(counts.entries())
      .map(([key, info]) => {
        ...
        return {
          name: { en, zh },
          count: info.count,
          tags: Array.from(info.tags).map(([tagEn, tagZh]) => ({ en: tagEn, zh: tagZh }))
        };
      })
      .sort((a, b) => {
...
async function generateAndSaveSummary() {
  try {
    ...
    const cocktailsPath = path.join(dataDir, 'cocktails.json');
    ...
    console.log('✅ Summary data generated and saved successfully');
  } catch (error) {

...
export function calculateDistance({
  ...
  if (selectedBaseSpirits.length > 0) {
    totalSelections++;
    if (selectedBaseSpirits.some(spirit =>
      cocktail.base_spirits.some(cocktailSpirit => 
  ...
  if (selectedIngredients.length > 0) {
    totalSelections++;
    if (selectedIngredients.some(ingredient =>
  ...
  if (selectedLiqueurs.length > 0) {
    totalSelections++;
    if (selectedLiqueurs.some(liqueur =>
      cocktail.liqueurs.some(cocktailLiqueur => 
  ...
  if (totalSelections > 0) {
    ingredientScore = 1000000 * (1 - (matchCount / totalSelections));
  }
  ...
  if (selectedFlavors.length > 0) {
    const matchingFlavors = selectedFlavors.filter(flavor => {
    ...
    flavorScore = 10000 * (1 - (matchingFlavors / selectedFlavors.length));
  }
  score += flavorScore;
  ...
  const attributeDistances = [
    sweetness !== null ? Math.pow(profile.sweetness - sweetness, 2) : 0,
    sourness !== null ? Math.pow(profile.sourness - sourness, 2) : 0,
    body !== null ? Math.pow(profile.body - body, 2) : 0,
    complexity !== null ? Math.pow(profile.complexity - complexity, 2) : 0,
    booziness !== null ? Math.pow(profile.booziness - booziness, 2) : 0
  ];

  const attributeScore = Math.sqrt(attributeDistances.reduce((sum, dist) => sum + dist, 0));
...
export function rankCocktails(
  ...
  const rankedCocktails: RankedCocktail[] = cocktails
    ...
    .filter(cocktail => cocktail.distance !== Number.MAX_SAFE_INTEGER) // Remove cocktails that don't meet mandatory criteria
  ...
  const scoredCocktails = rankedCocktails.map(cocktail => {
    ...
    if (cocktail.distance >= 1000000) {
      displayScore = Math.max(0, 60 - ((cocktail.distance - 1000000) / 20000));
    }
    // If there are ≥ 10,000 points, flavor mismatch is dominant factor
    else if (cocktail.distance >= 10000) {
      displayScore = Math.max(60, 80 - ((cocktail.distance - 10000) / 500));
    }
    // Otherwise attribute distance is dominant factor
    else {
      displayScore = Math.max(80, 100 - (cocktail.distance / 100));
    }

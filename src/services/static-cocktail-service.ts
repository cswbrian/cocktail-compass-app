...
import { Cocktail, CocktailPreview } from "@/types/cocktail";
...
class StaticCocktailService {
  ...
  private constructor() {
    if (StaticCocktailService.isInitialized) {
      console.warn('StaticCocktailService is being instantiated multiple times!');
    }
    
    // Initialize with decompressed data
    console.log('🔄 Decompressing static cocktails data...');
  ...
  public static getInstance(): StaticCocktailService {
    if (!StaticCocktailService.instance) {
      console.log('🎯 Creating new StaticCocktailService instance');
      StaticCocktailService.instance = new StaticCocktailService();
    }
  ...
  public getStaticCocktailBySlug(slug: string): Cocktail | undefined {
    return this.slugToCocktail.get(slug);
  }

  public getStaticCocktailById(id: string): Cocktail | undefined {
    return this.cocktails.find(cocktail => cocktail.id === id);
  }

  public getStaticCocktailsByFlavor(flavorSlug: string): Cocktail[] {
    // Check cache first
  ...
  public getStaticCocktailsByIngredient(ingredientSlug: string): Cocktail[] {
    // Check cache first
...
export const staticCocktailService = StaticCocktailService.getInstance(); 
  ...
  const handleSpiritButtonClick = (spirit: string) => {
    const matchingSpirits = summary.base_spirits
      .filter(item =>
        item.name.en
          .toLowerCase()
          .includes(spirit.toLowerCase()),
      )
      .map(item => item.name.en);

    // Toggle selection
    const isSelected = matchingSpirits.every(spirit =>
      selectedBaseSpirits.includes(spirit),
    );
    if (isSelected) {
      setSelectedBaseSpirits(
        selectedBaseSpirits.filter(
          s => !matchingSpirits.includes(s),
        ),
      );
    } else {
      setSelectedBaseSpirits([
        ...
        {spirits.map(spirit => {
          const matchingSpirits = summary.base_spirits
            .filter(item =>
              item.name.en
                .toLowerCase()
                .includes(spirit.en.toLowerCase()),
            )
            .map(item => item.name.en);
          const isSelected = matchingSpirits.every(s =>
            selectedBaseSpirits.includes(s),
          );
          ...
          return (
            <Button
              key={spirit.en}
              onClick={() =>
                handleSpiritButtonClick(spirit.en)
              }

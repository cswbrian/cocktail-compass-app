...
export function CocktailLogForm({
  ...
  const [cocktailNameInput, setCocktailNameInput] =
    useState('');
  ...
  const [isCreatingCustom, setIsCreatingCustom] =
    useState(false);
  const [customCocktailValues, setCustomCocktailValues] =
    useState<CustomCocktailValues | null>(null);
  const [visibility, setVisibility] = useState<
    'public' | 'private'
  >(
    existingLog?.visibility === 'private'
      ? 'private'
      : 'public',
  );
  ...
  useEffect(() => {
    if (existingLog) {
      const displayName = formatBilingualText(
        existingLog.cocktail.name,
        language,
      );
      ...
      setDrinkDate(
        existingLog.drinkDate
          ? new Date(existingLog.drinkDate)
          : undefined,
      );

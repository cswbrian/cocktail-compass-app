...
export function CustomCocktailModal({
  ...
  const [customCocktailName, setCustomCocktailName] = useState(initialName);
  const [customCocktailNameZh, setCustomCocktailNameZh] = useState(initialName);
  ...
  const handleCreateIngredient = async (ingredient: Ingredient) => {
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      {ingredients.map((ingredient) => (
                        <div key={ingredient.id} className="flex items-center justify-between bg-secondary/50 p-2 rounded">
                          <span>{formatBilingualText({ en: ingredient.nameEn, zh: ingredient.nameZh }, language)}</span>
                          <Button
                            variant="ghost"
                            size="sm"
                                ...
                                {searchResults.map((ingredient) => (
                                  ...
                                  >
                                    <span className="font-medium">
                                      {formatBilingualText({ en: ingredient.nameEn, zh: ingredient.nameZh }, language)}
                                    </span>
                                    <span className="text-sm text-muted-foreground">
                                      {t[ingredient.type === 'base_spirit' ? 'baseSpirit' : ingredient.type]}
                                    </span>
                                  </button>
                                ))}
              ...
              <div className="p-4 border-t mt-auto">
                <div className="flex w-full gap-2">
                  ...
                  <Button
                    ...
                    disabled={!customCocktailName || isLoading}
                    className="flex-1"
                  >
